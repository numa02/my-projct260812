...
- [ ] **TM-001** `mockups/timetable-master.html`にCSV/貼り付け取り込みセクションのモックアップを追加する
...
- [ ] **TM-002** `shared/parse-timetable-rows.ts`を実装する
...
- [ ] **TM-003** `shared/timetable-csv-validation.ts`を実装する
...
- [ ] **TM-004** `components/ui/PasteOrUploadArea.tsx`を汎用化し、既存の呼び出し元(生徒名簿)を追従させる
...
- [ ] **TM-005** `TimetableMasterForm.tsx`にCSV/貼り付け取り込みセクションを統合する
...
- [ ] **TM-006** `e2e/timetable-master.spec.ts`にCSV取り込みのE2Eケースを追加する
...
- [ ] **TM-007** `docs/features/timetable-master/test-cases.md`を更新する
