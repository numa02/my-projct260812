...
- [x] **PT-001** `shared/prompt-builder.ts`の初期値を新しい文面に差し替える
  - DoD: `DEFAULT_PROMPT_TEMPLATE`(学習用)・`DEFAULT_LIFE_PROMPT_TEMPLATE`(生活用)が教員から提示された文面になっている。「学期」の行は削除されている。プレースホルダーが`{{grade}}`・`{{targetCharCount}}`・`{{memos}}`の記法になっており、提示された文面の`{学年}`・`{文字数}`・`{メモ一覧}`がすべて置き換わっている。`{{pseudonymCode}}`は初期値に含まない
  - 依存: なし
...
- [x] **PT-008** 全体設計書・要件定義書を更新する
  - DoD: `docs/requirements.md`(F9・F10)、`docs/design/screens.md`(画面12・画面13)、`docs/design.md`(プロンプト組み立ての記載があれば)が実装と一致する。学期という概念を導入していないこと・仮名コードが初期値に含まれないことが要件側から読み取れる
  - 依存: PT-001, PT-004
