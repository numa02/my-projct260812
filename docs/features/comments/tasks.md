...
- [ ] **CM-001** バックアップ・リストア手順をローカル環境でリハーサルする
...
- [ ] **CM-002** `class`テーブルへのカラム追加マイグレーションを作成・ローカル適用する
...
- [ ] **CM-003(互換性レイヤー)** `student_comment`フェーズ1(拡張)マイグレーションを作成・ローカル適用する
...
- [ ] **CM-004** `tests/db/rls.test.ts`の`student_comment`関連テストをフェーズ1後のスキーマに追従させる
...
- [ ] **CM-005** `shared/schemas/index.ts`の`commentSaveInputSchema`から期間項目を除去する
...
- [ ] **CM-006** `hooks/useClassCommentPeriod.ts`を新規実装する
...
- [ ] **CM-007** `hooks/useStudentComments.ts`を生徒ごと単一所感に対応させる
...
- [ ] **CM-008** `components/comment/StudentCommentRow.tsx`を単一所感表示に変更する(CM-007と同一コミット)
...
- [ ] **CM-009** `mockups/comment.html`を更新する
...
- [ ] **CM-010** `app/(main)/comments/class/[[...id]]/ClassCommentsContent.tsx`を対象期間のクラス単位記憶に対応させる
