...
- [x] **CM-001** バックアップ・リストア手順をローカル環境でリハーサルする
  - DoD: ローカルの`supabase start`環境で`student_comment`にテストデータ(同一生徒に複数期間の所見を含む)を投入し、`pg_dump -t public.student_comment --data-only`でダンプ→`truncate table student_comment`→`psql -f`でリストア、の一連が成功することを確認する
  - 依存: なし
...
- [x] **CM-003(互換性レイヤー)** `student_comment`フェーズ1(拡張)マイグレーションを作成・ローカル適用する
  - DoD: `supabase/migrations/<timestamp>_student_comment_expand.sql`が、①`id`をタイブレーカーに含めた`DISTINCT ON`による生徒ごと最新1件へのDEDUP DELETE、②`period_start_date`/`period_end_date`のNOT NULL解除、③`unique (student_id)`制約の追加、の3ステップで作成される。ローカルで同一生徒に複数期間の所見を持つテストデータを投入した状態で`supabase db reset`を実行し、生徒ごとに1件へ集約されることをSQLクエリで確認する
...
- [x] **CM-007** `hooks/useStudentComments.ts`を生徒ごと単一所見に対応させる
...
- [x] **CM-008** `components/comment/StudentCommentRow.tsx`を単一所見表示に変更する(CM-007と同一コミット)
  - DoD: 対象期間との完全一致による`existing`/`history`振り分けロジック(L67-72相当)を削除し、`useStudentComments`が返す単一の所見をそのまま表示する。`historyOpen`のstate、および履歴セクション(L192-225相当、「過去の所見を見る」ボタンとその中身)を削除する。`periodStartDate`/`periodEndDate`のpropsは`CommentAiAssist`へのAI生成集計範囲として維持する。`npm run build`が通る
  - 依存: CM-007(同一コミット)
...
- [x] **CM-009** `mockups/comment.html`を更新する
  - DoD: 「過去の所見を見る」セクションを削除したモックアップに更新し、対象期間の入力欄に「クラスごとに自動保存されます」旨の注記を追加する。ブラウザで開いて崩れずに表示される
...
- [x] **CM-012** `docs/features/comments/test-cases.md`を更新する
  - DoD: 対象期間の自動保存・所見の単一化・バリデーションのP0/P1テストケースが追記され、履歴閲覧のケースが削除される
  - 依存: CM-011
  - ロールバック: ドキュメントのみのため無条件にrevert可能
  - 完了: コミット`b2cc757`

- [x] **CM-013** 本番へフェーズ1マイグレーション(CM-002, CM-003相当)を適用し、新コードをデプロイする
  - DoD: 本番DBの`student_comment`件数・生徒あたり最大所見件数を事前確認したうえで、CM-001の手順でバックアップを取得してからフェーズ1マイグレーションを本番適用する。適用後、Vercelへ新コードをデプロイし、所見の保存・表示、対象期間の自動保存・クラスごとの復元、AI生成が本番環境で正常に動作することを手動確認する
...
- [x] **CM-014(段階リリース・CM-013の数日後に実施)** `student_comment`フェーズ2(縮小)マイグレーションを作成・本番適用する
  ...
  - 補足(2026-09-20判明): 本タスクの本番適用はSupabase CLI(`db push`)を経由せずに行われたため、本番の`supabase_migrations.schema_migrations`に`20260918013828`の行が残っていなかった。DBの実体(期間列の削除・ユニーク制約の入れ替え)は適用済みだったため、`npx supabase migration repair --status applied 20260918013828`で履歴のみを補正した。以後、本番への適用はCLI経由で行い履歴を残す
  - ロールバック: **このタスク適用後はコードロールバックのみでは復旧できない。** CM-001で検証した手順に従い、まずカラムを復元する新規マイグレーションを適用してから、CM-013取得時点のバックアップをリストアする
  - 完了(2026-09-18): CM-013の適用(2026-09-11頃)から1週間以上経過し、ユーザーが本番の所見保存を実際に確認し問題なしと判断したことを受けて実施。コミット`379dd11`(`supabase/migrations/20260918013828_student_comment_contract.sql`)。実装時に、`export_teacher_data()` RPCが削除対象列を直接参照していたこと(未着手のまま列を削除すると本番のデータエクスポート機能が壊れる)、および`tests/db/schema.test.ts`・`tests/db/rpc-export.test.ts`が旧列を前提にしていたことが判明したため、同コミットで併せて修正した(design.md「影響を受ける既存テスト」表に追記済み)。適用前に本番で`student_comment`のバックアップを取得し(`pg_dump --data-only`)、適用後は本番へSQLを直接実行して`\d public.student_comment`でスキーマ(period列が存在せず`student_comment_student_id_key`のみ残る)を確認した。マイグレーション適用コマンド自体は自動安全装置(Production Deploy)によりエージェントからの直接実行がブロックされたため、ユーザー本人の端末から実行してもらい、出力を共有してもらう形で完了を確認した
