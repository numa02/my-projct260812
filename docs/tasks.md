...
- [x] **T-021** `export_teacher_data` RPC関数を実装する
  - DoD: 生徒ごとにメモ・所見がまとまったJSONが返り、`ai_provider_setting` が含まれない(F14)
...
- [x] **T-024b** 共通クラス選択フック `useClassOptions()` を実装する
  - DoD: 教員のクラス一覧取得+選択中クラスの生徒一覧取得をまとめて提供する。生徒名簿(T-055)・生徒別メモ一覧(T-064)・所見画面(T-065)の3箇所から共通で呼び出され、各画面で同じロジックを再実装しない
...
- [x] **T-056** 生徒名簿画面(一覧・編集・削除・仮名コード表示)(F2)
  - DoD: 生徒削除時に「メモ・所見も完全削除される」確認ダイアログが表示され、削除が反映される。生徒一覧の氏名部分をクリックすると、その生徒を選択済みの状態で生徒別メモ一覧画面に遷移する
...
- [x] **T-065** 所見画面の骨格(タブ切替・共通クラス/生徒選択)(F9, F10, F11)
  - DoD: 「生成」「履歴」の2タブを持つ1画面として実装され、画面内のクラス選択・生徒選択がタブ間で共有される(タブを切り替えても選択状態が保持される)。`useClassOptions()`(T-024b)を利用する
  - 注記: 画面への入口として、生徒名簿画面の各行に所見アイコンリンク(`/comments/students/[id]`、生徒別メモ一覧のリンクと同じ「[id]は初期選択のヒント」パターン)を追加した。screens.md/user-flow.mdには本画面への遷移導線が明記されていなかったため、design.mdのルーティング定義([id]付きの単一ルート)から妥当な入口として実装時に補った。生成・履歴タブの中身は準備中のプレースホルダーで、実装はT-066a/T-066b/T-067で行う
  - **2026-08-17追記**: 本タスクで実装した生徒単位・タブ切替の構成は、T-080でクラス単位の一覧編集画面(所見管理画面)に再設計され、置き換えられた。以下T-066a/T-066b/T-067の実装内容も同様
- [x] **T-066a** 所見画面・生成タブ:直接呼び出し(F9, F12)
  - DoD: 期間・目安文字数指定で生成され、AIの応答テキストがそのまま画面に表示される(仮名コード検出・実名置換は行わない)。認証/レート制限エラー時は所見が保存されない
...
- [x] **T-066b** 所見画面・生成タブ:プロンプトコピー運用(F10, F12)
  - DoD: プロンプトのコピー、外部AIの応答を貼り付け欄に入力してそのまま所見として保存できる(仮名コード検出・実名置換は行わない)
  - 注記: F12の「初めて実行する際の注意文」はAPIキー設定済み(直接呼び出し)・未設定(プロンプトコピー)のどちらの分岐でも常時表示する形に簡略化した(「初回のみ」を判定する永続化の仕組みは設計されていないため)
- [x] **T-067** 所見画面・履歴タブ(F11)
  - DoD: 期間別一覧、上書き確認ダイアログ、手動作成が動作する(クラス・生徒選択はT-065のタブ共通stateを使う)
  - 注記: 既存所見の編集時は期間(開始日・終了日)を読み取り専用にし、作成方法(direct_ai/prompt_copy/manual)は編集前の値を保持したまま内容のみ更新する仕様とした(要件定義書F11の「編集して保存すると内容が更新される」に基づく解釈)。上書き確認は新規手動作成が既存の期間と重複した場合のみ発生させ、既存レコードの編集(同一レコードの更新)では発生させない
...
- [x] **T-070** 設定画面・データエクスポート(F14)
  ...
  - 注記: `export_teacher_data()` RPC(T-021で実装済み、生徒ごとにメモ・所見がまとまりAIプロバイダ設定を含まない)をそのまま呼び出すだけのため、本タスクは確認ダイアログ+ダウンロード処理のフロントのみ実装した
...
- [x] **T-072** 全画面のEmptyState表示を確認・調整する(`docs/design/screens.md` の空状態一覧に対応)
  - 注記: 実装済み全画面のEmptyStateメッセージをscreens.mdの空状態一覧と突き合わせ、文言が完全に一致していることを確認した(クラス管理・生徒名簿・科目管理・週次時間割・授業記録・生徒別メモ一覧・所見画面生成/履歴タブ)。追加修正は不要だった
- [x] **T-073** 全画面のローディング状態を確認・調整する
  - 注記: 全画面でLoadingSpinner/Skeletonによる初期読み込み表示、ボタンの`loading`状態によるミューテーション中表示が一貫して実装されていることを確認した。所見生成(直接呼び出し)はAI応答待ちが数秒〜数十秒かかる想定のため、ボタンのスピナーに加えて状況を示すラベル付きのLoadingSpinnerを追加した(`components.md`のLoadingSpinner使用箇所の指定に対応)
- [x] **T-074** アクセシビリティレビュー(コントラスト比WCAG AA、タップ領域44×44px、色以外での情報伝達)
  - 注記: (1) タップ領域: テーブル・カード内のアイコンのみボタン(編集・削除・所見アイコン等、計12箇所)とModalの閉じるボタンが実測24×24px程度だったため、44×44pxに拡大した。週次時間割の週送りボタン等、テキスト付きボタンや影響範囲の大きい要素は現状維持。(2) コントラスト比: design-system.mdの色トークンを白背景上の通常テキストとして検証したところ、`gray-500`・`success-500`・`warning-500`がWCAG AA(4.5:1)未達(3.3〜3.4:1、3:1の大文字/UI部品基準は満たす)だったため、色相を保ったまま暗く調整し4.5:1以上を確保した(`gray-700`・`gray-900`・`error-500`は当初から適合)。(3) 色以外での情報伝達: Badgeコンポーネントは元々「色のみに依存せず必ずラベルテキストを併記する」設計であり、共有区分・作成方法等はすべてテキスト併記済みであることを確認
...
- [x] **T-077** E2Eゴールデンパスを実装する(サインアップ→クラス作成→CSV登録→時間割設定→メモ記録→所見生成・保存)
...
- [x] **T-080** 所見画面をクラス単位の一覧編集画面(所見管理画面)に再設計する(F9, F10, F11)
  - 背景: T-065〜T-067で実装した「クラスを選んで生徒を1人選び、タブで生成/履歴を切り替える」構成は、学期末に生徒全員分の所見をまとめて書きたいという実際の使い方に対して手数が多かった。ユーザーから「所見はクラス単位で編集していきたい。名前と所見を記載するスペースを初期表示にして、AI生成やプロンプトが必要な場合はクリックやタップで開くようにしてほしい」という要望を受け、事前に4点(画面構成/対象期間の扱い/既存詳細画面の扱い/AI生成の単位)をヒアリングした上で再設計した
  - DoD: `/comments/class/[[...id]]`でクラス+対象期間(開始日・終了日、画面全体で1つ)を指定すると、そのクラスの生徒全員分の行が一覧表示される。各行は氏名・所見入力欄(常時表示)・保存ボタンを持ち、「AIで生成する」(F9直接呼び出し/F10プロンプトコピー運用)と「過去の所見を見る」(F11履歴、対象期間以外の保存済み所見を閲覧)は折りたたみで必要な時だけ開く。生徒名簿画面の所見アイコンからは対象クラス+対象生徒のハイライト付きで遷移する。ナビゲーションメニューにも「所見管理」を追加した
  - 決定事項(ヒアリング結果): (1) 画面構成は授業記録画面(F6)と同じ折りたたみ一覧パターンを採用、ただし氏名+所見入力欄は常時表示という点が異なる。(2) 対象期間はクラス全体で1つだけ指定し、生徒ごとの個別期間指定は廃止。(3) 生徒単位の詳細画面(`/comments/students/[id]`、生成/履歴タブ)は本画面に統合し廃止。(4) AI生成は生徒ごとに個別実行(クラス一括生成は今回未実装、必要になれば別途検討)
  - 設計変更に伴う仕様変更: 対象期間が画面上部で編集前から一意に確定するため、F11の上書き確認ダイアログ(「この期間の所見は既に保存されています。上書きしますか」)が不要になり廃止した(保存操作自体が既存レコードへの意図的な更新になるため)。`docs/requirements.md` F9-F11、`docs/design/screens.md`画面12、`docs/design/user-flow.md`フロー4、`docs/design.md`(ルーティング・コンポーネント表・§5.4直接アクセスパターン例)を合わせて更新した
  - 実装: `components/comment/StudentCommentRow.tsx`(行本体、`useStudentComments()`を行ごとに呼び出し対象期間の既存所見初期表示+履歴一覧の両方に使う)、`components/comment/CommentAiAssist.tsx`(AI生成セクション、結果は保存せず行の所見入力欄にコールバックで反映するだけ)、`app/(main)/comments/class/[[...id]]/`(page.tsx, ClassCommentsView.tsx, ClassCommentsContent.tsx)。旧`app/(main)/comments/students/[id]/`一式と`CommentGenerateTab.tsx`/`CommentHistoryTab.tsx`は削除
...
- [ ] **B-005** 所見管理画面でのクラス一括AI生成(現状は生徒ごとに個別実行のみ。T-080で見送り)
- [ ] **B-006** エクスポート内容(項目・形式)の決定と実装 ── 現状は`export_teacher_data`の結果を全データ1ファイルのJSONとしてダウンロードするだけで、実務での使い道(通知表への貼り付け等)に合わせた設計をしていない。所見だけを抽出する、CSVを選べるようにする、生活メモと授業メモを分ける、などの要否を決めてから実装する
