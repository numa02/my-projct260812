...
flowchart TD
    ...
    O --> P[その行の保存を実行]
    P --> R[その生徒・対象期間の所見として保存・作成方法を記録]
    R --> T[保存済みバッジ・最終更新日時が行に反映される(同一画面内)]
```

**補足**: 手動作成(F9・F10を経由しない直接入力)も、所見入力欄に直接テキストを入力して保存するだけで同じパスをたどる。対象期間は画面上部で編集前から一意に確定しているため、上書き確認ダイアログは発生しない(生徒の行を開いた時点でその期間の既存所見が初期表示され、保存は常にその期間への意図的な更新になるため)。他の期間の所見を見る場合は、その生徒の行の「過去の所見を見る」を開く(閲覧専用。編集する場合は画面上部の対象期間を変更する)。
