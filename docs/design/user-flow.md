...
学期末の所感作成前などに、特定生徒のメモを確認・整理する流れ。
...
学期末に所感文をAI生成(直接呼び出し／プロンプトコピー運用)または手動作成し、保存するまでの流れ。所感はクラス単位の一覧画面(所感管理画面)で、対象期間を1つ指定してクラス全員分をまとめて編集する。

```mermaid
flowchart TD
    A[所感管理画面を開く:クラスと対象期間を指定] --> A2[クラス全員の行が一覧表示される]
    ...
    G -->|成功| H[応答テキストをその生徒の所感入力欄に反映]
    G -->|認証/レート制限エラー| I[エラー表示・所感は保存されない]

    B -->|No:プロンプトコピー運用| K[プロンプト表示・コピー]
    K --> L[外部AIチャットに貼り付けて応答を取得]
    L --> M[応答を貼り付け欄に入力し「所感欄に反映」]
    M --> H

    H --> O[所感入力欄で内容を確認・必要に応じて編集]
    O --> P[その行の保存を実行]
    P --> R[その生徒・対象期間の所感として保存・作成方法を記録]
    R --> T[保存済みバッジ・最終更新日時が行に反映される(同一画面内)]
```

**補足**: 手動作成(F9・F10を経由しない直接入力)も、所感入力欄に直接テキストを入力して保存するだけで同じパスをたどる。対象期間は画面上部で編集前から一意に確定しているため、上書き確認ダイアログは発生しない(生徒の行を開いた時点でその期間の既存所感が初期表示され、保存は常にその期間への意図的な更新になるため)。他の期間の所感を見る場合は、その生徒の行の「過去の所感を見る」を開く(閲覧専用。編集する場合は画面上部の対象期間を変更する)。
...
flowchart TD
    ...
    subgraph AI呼び出しエラー
        C1[所感生成:直接呼び出し] --> C2{APIエラー発生?}
        C2 -->|認証エラー/レート制限| C3[エラー表示・所感は未保存のまま維持]
        C2 -->|エラーなし| C4[生成成功]
    end
