...
erDiagram
    ...
    CLASS {
        ...
        date comment_period_start_date "所見管理画面の対象期間(開始日)。nullable。docs/features/comments/design.md参照"
    ...
    STUDENT_COMMENT {
        uuid id PK
        uuid student_id FK "unique。学習の所見を生徒ごとに最新1件のみ保持(docs/features/comments/design.md参照)"
        text content
...
**科目マスタの初期値は解消済み**: サインアップ画面で学校区分(小学校/中学校)を選ぶと、対応する標準科目セット(小学校10科目/中学校11科目)が自動登録される。学校区分を選ばなければ従来通り空の状態で始まる。既存アカウントも科目管理画面から任意のタイミングで同じセットを追加できる。詳細は`docs/features/subjects/requirements.md`・`docs/features/subjects/design.md`。
