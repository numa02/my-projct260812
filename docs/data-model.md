...
F14は既存エンティティを横断的に読み取ってJSONにまとめる機能であり、新しいエンティティは追加していない。目的は生徒ごとのメモ・所感を長期的に手元へ保管しておくことにあるため、出力は生徒ごとにメモ・所感がまとまる形にする。`AI_PROVIDER_SETTING`(APIキーを含む)はエクスポート対象から除外する。インポート(復元)機能は持たないため、エクスポートしたファイルからDBの状態を再現する経路は現時点で存在しない。バックアップ目的で定期的にエクスポートを促す仕組み(リマインダー等)は要件に含まれておらず、教員が自発的に実行しなければ手元にファイルは残らない。
