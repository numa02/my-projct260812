...
学校教員向けの週間時間割・生徒メモ・所見自動生成ツール。教員が授業ごとに生徒別メモを記録し、学期末にAIが所見文の下書きを生成する。教員1人につき1アカウント、他教員とのデータ共有は行わない(`docs/requirements.md` 参照)。
...
- 単純CRUD(クラス表示名編集、生徒編集・削除、科目名編集、メモ・所見の保存など)→ `supabase-js` から直接 `insert`/`update`/`delete`/`upsert`
...
- ただしバックグラウンドジョブとして動作している場合は、上記に関わらずツール側でworktreeでの分離が強制されることがある(共有チェックアウトへの編集・`git pull`等が拒否される)。その場合はworktreeで作業するか、コマンドをユーザーに渡して本人の端末で実行してもらう
