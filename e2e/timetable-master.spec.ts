...
/** 30マス分の「曜日,時限,科目名,クラス名」CSVテキストを組み立てる(教科担任制モード用のテスト) */
function build30RowCsv(subjectName: string, className: string): string {
  const rows: string[] = ["曜日,時限,科目名,クラス名"];
  for (const weekday of WEEKDAYS) {
    for (const period of PERIODS) {
      rows.push(`${weekday},${period},${subjectName},${className}`);
    }
  }
  return rows.join("\n");
}
...
async function importCsvViaPaste(page: Page, csvText: string): Promise<void> {
  ...
  await page.getByLabel(/時間割表|曜日,時限,科目名/).fill(csvText);
...
test.describe("時間割マスタ設定画面(T-058, T-059, T-060)", () => {
  ...
  test("CSV取り込み(教科担任制モード): 30マス分の4列CSVを貼り付けると科目・クラスが両方反映される", async ({
    ...
    await page.goto("/timetable/master");
    await page.getByRole("radio", { name: "教科担任制モード" }).click();
    await importCsvViaPaste(page, build30RowCsv("国語", "1年1組"));
  ...
  test("CSV取り込み: 科目名が登録済み科目と一致しない場合はエラーになりグリッドは変更されない", async ({
    ...
    await page.goto("/timetable/master");
    await page.getByLabel("クラス(全マスに適用)").selectOption({ label: "1年1組" });
    // 30マス中1マスだけ科目名を不正にする(残り29マスは正しい「国語」)
    const gridText = build30CellGrid("国語").replace("国語", "存在しない科目");
    await importCsvViaPaste(page, gridText);
  ...
  test("CSV取り込み(一括モード): 時間割表をそのまま貼り付けると(曜日5列×時限6行、科目名のみ)全マスが反映される", async ({
    page,
  }) => {
    await signUpAndLogin(page);
    await createClass(page, "1", "1年1組");
  ...
  test("CSV取り込み: 30マスに満たないデータはINCOMPLETEエラーになる", async ({ page }) => {
    ...
    const incompleteGrid = "国語\t国語\t国語\t国語\t国語";
    await importCsvViaPaste(page, incompleteGrid);
